# ship_system/src/controls/motion_controller.py
...
class MotionController:
    def __init__(self):
        ...
        self.pid_pitch = MiniPID(kp=2.5,  ki=0.05, kd=1.5,  output_limits=(-150, 150))
    ...
    def process_and_send(self, cmds: dict, sensor_data: dict, attitude_data: dict,
        ...
        if software_kill_active:
            ...
            return {
                "ballast_pct": 0, "ballast_status": "KILL",
        ...
        if cmds.get("is_autonomous", False) and target_depth_auto is not None:
            pwm_speed = int(self.pid_depth.compute(target_depth_auto, current_depth))
        else:
            if abs(ballast_cmd) > 15:
                pwm_speed = int(ballast_cmd)

        stm32.send_manual_speed(pwm_speed)
        stm32.send_target_position(0, gripper_cmd)
        ...
        # --- KALKULASI PITCH & HEAVE ---
        raw_p = float(cmds.get("pitch", 0))

        # Transisi toggle tombol hold pitch
        if hold_pitch_toggle and not self.prev_hold_pitch_toggle:
            self.target_pitch = current_pitch
            self.pitch_baseline = self.last_pitch_effort
            self.pid_pitch.reset()
            if logger:
                logger.push(f"[CTRL] Pitch Hold AKTIF di {self.target_pitch:.1f}°", "ctrl")
        elif not hold_pitch_toggle and self.prev_hold_pitch_toggle:
            self.target_pitch = 0.0
            self.pitch_baseline = 0.0
            self.pid_pitch.reset()
            if logger:
                logger.push("[CTRL] Pitch Hold NONAKTIF -> Target kembali 0.0°", "ctrl")
        ...
        if abs(raw_p) > 50:
            ...
            effort = self.pid_pitch.compute(self.target_pitch, current_pitch)
            pitch_axis = self.pitch_baseline + effort
            self.last_pitch_effort = pitch_axis
        ...
        if abs(raw_y) > 50:
            if self.heading_hold_active and logger:
                logger.push(f"[CTRL] Belok manual (Yaw: {int(raw_y)})", "ctrl")
            self.heading_hold_active = False
            ...
            if not self.heading_hold_active:
                ...
                self.pid_yaw.reset()
                if logger:
                    logger.push(f"[CTRL] Heading Hold TERKUNCI di {self.target_heading:.1f}°", "ctrl")

            err_heading = (self.target_heading - current_heading + 180) % 360 - 180
            yaw_axis = self.pid_yaw.compute(0, -err_heading)

        surge_axis = (float(cmds.get("surge", 0)) / 1000.0) * self.SURGE_MAX_OFFSET

        pixhawk.send_movement_target(
            pitch=1500, roll=1500,
            heave=clamp(1500 + pitch_axis),
            yaw=clamp(1500 + yaw_axis),
            surge=clamp(1500 + surge_axis)
        )
